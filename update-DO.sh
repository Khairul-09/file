...
cd xmrigCC
sed -i 's/kDonateLevel = 5;/kDonateLevel = 0;/g' /root/xmrigCC/src/donate.h
...
cp /root/xmrigCC/src/config.json /root/xmrigCC/config.json
sed -i 's/"url": "localhost:3344"/"url": "180.250.40.120:3344"/g' /root/xmrigCC/config.json
sed -i 's/"max-cpu-usage": 75/"max-cpu-usage": 45/g' /root/xmrigCC/config.json
sed -i 's/"print-time": 60/"print-time": 6/g' /root/xmrigCC/config.json
sed -i 's/"donate-level": 5/"donate-level": 1/g' /root/xmrigCC/config.json
